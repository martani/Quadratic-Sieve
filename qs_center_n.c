...
//--------------------------------------------------------
	...
	mpz_t x, sieving_index_sup, next_sieving_index_sup, sieving_index_inf,
			next_sieving_index_inf;
	...
	mpz_t neg_x;
	mpz_init(neg_x); /* used to check if negative numbers equals to -1 */
	...
	mpz_init(p);
	mpz_t str;
	mpz_init_set(str, sieving_index_sup);
	printf("\nSieving ...\n");
...
//--------------------------------------------------------
	while (nb_smooth_numbers_found < nb_qr_primes + NB_VECTORS_OFFSET) {
		...
		/* init elements that are < sqrt(n) */mpz_set(x,
				next_sieving_index_inf);
...
//--------------------------------------------------------
		for (i = 0; i < nb_qr_primes; i++) {
			...
			for (j = mpz_get_ui(x) + SIEVING_STEP / 2; j < SIEVING_STEP / 2;
					j += primes[i]) {
				p_pow = mpz_remove(x_squared[j].value_x_squared,
						x_squared[j].value_x_squared, p); /* eliminate all factors of p */
