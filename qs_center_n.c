...
/*-------------- This version sieves in an interval centered at sqrt(N) --------------*/
...
{
	char s[512];
    sprintf(s, "%s_%s_center", file_name, mpz_get_str(NULL, 10, N));
...
int main(int argc, char **argv)
{
    ...
    mpz_t sqrtN, rem; mpz_init(sqrtN); mpz_init(rem);
...
//--------------------------------------------------------
    ...
    mpz_t tmp, r1, r2; mpz_init(tmp); mpz_init(r1); mpz_init(r2);
    ...
    mpz_clear(tmp); mpz_clear(r1); mpz_clear(r2);
...
//--------------------------------------------------------
    START_TIMER();
    init_matrix(&matrix, nb_qr_primes + NB_VECTORS_OFFSET + 1, nb_qr_primes);       /* +1 bit for the (negative) sign since we go under sqrt(N) in this version */
...
//--------------------------------------------------------
    ...
    mpz_t x, sieving_index_sup, next_sieving_index_sup, sieving_index_inf, next_sieving_index_inf;
    unsigned long ui_index, SIEVING_STEP = 600000;              /* we sieve for 60000 elements at each loop */
    uint64_t p_pow;
    smooth_number_t *x_squared;			/* array used for temporal sieving */
    
    x_squared = calloc(SIEVING_STEP, sizeof(smooth_number_t));
    smooth_numbers = calloc(nb_qr_primes + NB_VECTORS_OFFSET, sizeof(smooth_number_t));

    mpz_t neg_x; mpz_init(neg_x);               /* used to check if negative numbers equals to -1 */
    mpz_init_set(x, sqrtN);
    mpz_init_set(sieving_index_sup, x);
    mpz_init_set(next_sieving_index_sup, x);

    mpz_sub_ui(x, x, 1);
    mpz_init_set(sieving_index_inf, x);
    mpz_init_set(next_sieving_index_inf, x);
    
    mpz_t p; mpz_init(p);
    mpz_t str; mpz_init_set(str, sieving_index_sup);
    printf("\nSieving ...\n");

//--------------------------------------------------------
// Init before sieving
//--------------------------------------------------------
    /* now we have two sieving sides, less that sqrt(n) and greater that sqrt(n) */
    for(i=0; i < SIEVING_STEP; i++)
    {
        ...
        mpz_init2(x_squared[i].factors_vect, nb_qr_primes + 1);
    }
...
while(nb_smooth_numbers_found < nb_qr_primes + NB_VECTORS_OFFSET)
{
    ...
    mpz_set(x, next_sieving_index_sup);					/* sieve numbers from sieving_index to sieving_index + sieving_step */
    mpz_set(sieving_index_sup, next_sieving_index_sup);

    printf("\r");
    printf("\t\tSieving at: %s30 <--> Smooth numbers found: %" PRId64 "/%" PRId64 "",
            mpz_get_str(NULL, 10, sieving_index_sup),
            nb_smooth_numbers_found, nb_qr_primes);
    fflush(stdout);

    for(i=0; i < SIEVING_STEP/2; i++)									/* init elements that are > sqrt(n) */
    {
        ...
        mpz_clear(x_squared[i].factors_vect);                   /* reconstruct a new fresh 0ed vector of size nb_qr_primes bits + 1 */
        mpz_init2(x_squared[i].factors_vect, nb_qr_primes + 1);
        mpz_clrbit(x_squared[i].factors_vect, 0);               /* set the first bit to 0 for positive numbers */
        
        mpz_add_ui(x, x, 1);
    }
    mpz_set(next_sieving_index_sup, x);
    
    /* init elements that are < sqrt(n) */
    mpz_set(x, next_sieving_index_inf);
    mpz_set(sieving_index_inf, next_sieving_index_inf);
    for(i=SIEVING_STEP/2; i < SIEVING_STEP; i++)
    {
      mpz_set(x_squared[i].value_x, x);
      ...
      mpz_clear(x_squared[i].factors_vect);                   /* reconstruct a new fresh 0ed vector of size nb_qr_primes + 1 bits*/
      mpz_init2(x_squared[i].factors_vect, nb_qr_primes + 1);
      mpz_setbit(x_squared[i].factors_vect, 0);               /* set the first bit to 1 for negative numbers */
      
      mpz_sub_ui(x, x, 1);
    }
    mpz_set(next_sieving_index_inf, x);
...
//--------------------------------------------------------
    for(i=0; i<nb_qr_primes; i++)
    {
        ...
        mpz_sub(x, x, sieving_index_sup);                           /* x contains index of first number that is divisible by p */

        for(j = mpz_get_ui(x); j < SIEVING_STEP/2; j += primes[i])
        {
        ...
        mpz_set(x, next_sieving_index_inf);
        get_sieving_start_index(x, x, p, modular_roots[i].root1);
        mpz_sub(x, x, next_sieving_index_inf);
        
        /* sieve in the direction < sqrt(n) */
        for(j = mpz_get_ui(x) + SIEVING_STEP/2; j < SIEVING_STEP/2; j += primes[i])
        {
          p_pow = mpz_remove(x_squared[j].value_x_squared, x_squared[j].value_x_squared, p);      /* eliminate all factors of p */
          ...
          mpz_mul_si(neg_x, x_squared[j].value_x_squared, -1);
          if(mpz_cmp_ui(neg_x, 1) == 0)
          {
            save_smooth_number(x_squared[j]);
            nb_smooth_per_round++;
          }
    ...
    /*sprintf(s, "[start: %s - end: %s - step: %" PRId64 "] nb_smooth_per_round: %d",
           mpz_get_str(NULL, 10, sieving_index_sup),
           mpz_get_str(NULL, 10, next_sieving_index_sup),
           SIEVING_STEP,
           nb_smooth_per_round);
...
//--------------------------------------------------------
    ...
    free(primes);
    /********************** clear mpz _t **********************/
    mpz_clear(B); mpz_clear(N);
    sqrtN, rem;
    mpz_clear(x);
    mpz_clear(sieving_index_sup);
    mpz_clear(next_sieving_index_sup);
    mpz_clear(sieving_index_inf);
    mpz_clear(next_sieving_index_inf);
    mpz_clear(p);
    mpz_clear(str);
    /********************** clear mpz _t **********************/
