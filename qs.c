...
uint64_t next_smooth_nb_index = 0;			/* next smooth number to store */
uint64_t nb_qr_primes;						/* number of primes p where n is a quadratic residue mod p*/
uint64_t *primes;
smooth_number *smooth_numbers;
int NB_VECTORS_OFFSET = 3;		/* */
...
//TODO save smooth numbers on disk, just save the exponents in GF2 in the matrix
mpz_t tmp_matrix_row;
void save_smooth_number(smooth_number n)
{
	mpz_clear(tmp_matrix_row);										/* tmp_matrix_row must be initialized already */
	mpz_init2(tmp_matrix_row, nb_qr_primes);

	if(next_smooth_nb_index > nb_qr_primes + NB_VECTORS_OFFSET)		/* if we have sufficient smooth numbers, skip saving */
		return;
	...
	smooth_numbers[next_smooth_nb_index++] = tmp;
...
int main(int argc, char **argv)
{
	...
	/* caculate the smoothness base for the given N */
	get_smoothness_base(B, N);
	uBase = mpz_get_ui(B);
...
//[Sieving]
	...
	uint64_t p_pow;
	smooth_number *x_squared;

	x_squared = calloc(SIEVING_STEP, sizeof(smooth_number));
	smooth_numbers = calloc(nb_qr_primes + NB_VECTORS_OFFSET, sizeof(smooth_number));
	...
	mpz_t p; mpz_init(p);
	mpz_t str; mpz_init_set(str, sieving_index);
	printf("\nSieving ...\n");
...
//Init before sieving
	...
	printf("\n");
/* WHILE smooth numbers founds is less that the primes in the smooth base + NB_VECTORS_OFFSET */
while(next_smooth_nb_index < nb_qr_primes + NB_VECTORS_OFFSET)
{
	/* sieve numbers from sieving_index to sieving_index + sieving_step */
	mpz_set(x, next_sieving_index);
	mpz_set(sieving_index, next_sieving_index);

	/*printf("Calculating x²-n [start index: %s] [nb items: %u]...\n",
				mpz_get_str(NULL, 10, sieving_index),
				SIEVING_STEP);*/
	printf("\r");
	printf("\t\tSieving at: %s30 <--> Smooth numbers found: %" PRId64 "/%" PRId64 "",
			mpz_get_str(NULL, 10, sieving_index),
			next_smooth_nb_index, nb_qr_primes);
	fflush(stdout);
	...
	for(i=0; i < SIEVING_STEP; i++)
	{
		...
		/* calculate value_x_squared <- x²-n */
		mpz_pow_ui(x_squared[i].value_x_squared, x, 2);
	...
	/*for(j=0, i=0; i<next_smooth_nb_index; i++)
	{
	...
	/* the matrix ready, start gauss elimination
	 * we fill the matrix on the call of save_smooth_number() */
	START_TIMER();
